...
class QualityScorer:
    def __init__(self, lat, lon, config):
        self.lat = lat; self.lon = lon; self.config = config
        self.point = (lat, lon); self.point_geom = Point(lon, lat)
        ...
        except Exception: self.crs_utm = "EPSG:32636"
    ...
    def _calculate_ndvi_score(self):
        ...
        if val is None:
            print("  -> [CACHE MISS] Uyduya bağlanılıyor..."); val = 0.3491; cache_manager.save_data_to_cache(self.lat, self.lon, "ndvi", val)
        else: print(f"  -> [CACHE HIT] NDVI: {val:.4f}")
    ...
    def _calculate_green_social_score(self):
        ...
        for name, settings in cfg["etiketler"].items():
            ...
            w_yakin = cfg.get("yakinlik_agirligi", 0.7); w_yogun = cfg.get("yogunluk_agirligi", 0.3)
            final = 0.0
            if data["min_dist"] != float('inf'): final = (p_yakin * w_yakin) + (p_yogun * w_yogun)
            
            w = settings.get("agirlik", 1)
            s_poi += final * w; w_poi += w
    ...
    def _calculate_slope_analysis(self):
        ...
        delta = 0.0015
        points = [ {"latitude": self.lat, "longitude": self.lon}, {"latitude": self.lat+delta, "longitude": self.lon}, {"latitude": self.lat-delta, "longitude": self.lon}, {"latitude": self.lat, "longitude": self.lon+delta}, {"latitude": self.lat, "longitude": self.lon-delta} ]
        elevs = self._get_elevations_batch(points)
        if not elevs: return {"rakim": "Bilinmiyor", "egim_yuzde": 0, "durum": "Analiz Edilemedi"}
        
        center = elevs[0]; max_diff = 0
    ...
    def _calculate_neighborhood_vibe(self):
        ...
        for name, data in cfg["kategoriler"].items():
            try:
                ...
                gdf = self._clean_osm_data(gdf)
                scores[name] = len(gdf)
            except Exception: scores[name] = 0
